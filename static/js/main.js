...
function tryLogin() {
        ...
        function switchCalcMode(mode) {
            ...
            if(mode === 'quick') {
                document.getElementById('modeQuick').style.display = 'block';
                document.getElementById('modeRows').style.display = 'none';
                
                document.getElementById('tabQuick').style.background = 'var(--primary)';
                document.getElementById('tabQuick').style.color = '#000';
                document.getElementById('tabRows').style.background = 'transparent';
                document.getElementById('tabRows').style.color = 'var(--text-main)';
                evalQuickCalc(); 
            } else {
                document.getElementById('modeQuick').style.display = 'none';
                document.getElementById('modeRows').style.display = 'block';
                
                document.getElementById('tabRows').style.background = 'var(--primary)';
                document.getElementById('tabRows').style.color = '#000';
                document.getElementById('tabQuick').style.background = 'transparent';
                document.getElementById('tabQuick').style.color = 'var(--text-main)';
                calcSubtotals(); 
            }
        }

        function openCalcModal(targetId) { 
            calcTargetInputId = targetId; 
            document.getElementById('quickCalcInput').value = '';
            document.getElementById('calcRowsContainer').innerHTML = '';
            addCalcRow();
            document.getElementById('calcGrandTotal').innerText = '0';
            document.getElementById('calcModal').style.display = 'flex'; 
            switchCalcMode('quick'); 
            setTimeout(() => document.getElementById('quickCalcInput').focus(), 100);
        }
        
        function closeCalcModal() { document.getElementById('calcModal').style.display = 'none'; }
        
        function evalQuickCalc() {
            if(currentCalcMode !== 'quick') return;
            let input = document.getElementById('quickCalcInput').value;
            let sanitized = input.replace(/[^0-9+\-*/.]/g, '');
            try {
                if (sanitized) {
                    let res = new Function('return ' + sanitized)();
                    if(isFinite(res)) document.getElementById('calcGrandTotal').innerText = Math.floor(res);
                } else { document.getElementById('calcGrandTotal').innerText = '0'; }
            } catch(e) {}
        }

        function addCalcRow() {
            let div = document.createElement('div'); div.className = 'calc-row';
            div.innerHTML = `<input type="number" class="glass-input calc-val" autocomplete="off" placeholder="القيمة" oninput="calcSubtotals()" style="flex:1.5;"><select class="glass-input calc-op" onchange="calcSubtotals()" style="flex:1;"><option value="*">✖️ ضرب</option><option value="/">➗ قسمة</option></select><input type="number" class="glass-input calc-price" autocomplete="off" placeholder="السعر" oninput="calcSubtotals()" style="flex:1.5;"><div style="flex:1.5; text-align:center; font-weight:900; color:var(--warning); font-size:14px;"><span class="calc-sub">0</span></div><button class="btn-mini b-full" onclick="this.parentElement.remove(); calcSubtotals();">❌</button>`;
            document.getElementById('calcRowsContainer').appendChild(div);
        }
        
        function calcSubtotals() {
            if(currentCalcMode !== 'rows') return;
            let grandTotal = 0;
            document.querySelectorAll('.calc-row').forEach(row => {
                let v = parseFloat(row.querySelector('.calc-val').value) || 0; 
                let pStr = row.querySelector('.calc-price').value; 
                let op = row.querySelector('.calc-op').value; 
                let sub = 0;
                if (v > 0) { 
                    if (pStr === '') { sub = v; } 
                    else { 
                        let p = parseFloat(pStr) || 0; 
                        if (p > 0) { if(op === '*') sub = v * p; else if(op === '/') sub = v / p; } 
                    } 
                }
                sub = Math.floor(sub); 
                row.querySelector('.calc-sub').innerText = sub; 
                grandTotal += sub;
            });
        ...
        function applyCalcTotal() {
            let total = document.getElementById('calcGrandTotal').innerText;
            if(total !== '0' && total !== 'NaN' && calcTargetInputId) { 
                let target = document.getElementById(calcTargetInputId);
                if(target) { target.value = total; target.focus(); showToast("تم نقل النتيجة.", "success"); }
            }
            closeCalcModal();
        }
        ...
        // ========== سلة المحذوفات الدائمة ==========
        // تُخزَّن في sysDB.trash_bin - آخر 25 عملية حذف
        if(!sysDB.trash_bin) sysDB.trash_bin = [];
        ...
        function addToTrashBin(item, dbKey, sectionName) {
            ...
            if(sysDB.trash_bin.length > 25) sysDB.trash_bin.pop();
            saveDB();
        }
   ...
   function renderTrashContent() {
    ...
    // 2. حالة استرجاع "تسلسل" (عملية فرعية) لشركات أو كبار عملاء
    ...
    // 3. حالة استرجاع "حساب شركة" كامل أو كبار عملاء
    ...
    // 4. الحالة العادية
        ...
        if(arr) {
            if (!entry.item.id || !arr.some(x => x.id === entry.item.id)) {
                arr.push(entry.item);
                // استخدام [...arr] لضمان تحديث الواجهة بشكل سليم 100%
                setDbArr(entry.dbKey, [...arr]);
                restoredName = entry.item.name || "عنصر";
                restored = true; 
            } else {
            ...
            showToast("❌ لم يتم العثور على القسم لإرجاع العنصر!", "error");
            return;
        }
    }

    if (!restored) {
        showToast("❌ فشل الاسترجاع، الرجاء التأكد من البيانات", "error");
        return;
    }

    // تنظيف السلة بعد الاسترجاع الناجح
    sysDB.trash_bin.splice(idx, 1);
    
    logAction(`استرجاع [${restoredName}] من سلة المحذوفات.`);
    saveDB();
    renderActiveSection();
    renderTrashContent();
